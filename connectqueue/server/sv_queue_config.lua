...
Config.Language = {
    ...
    connecting = "\xE2\x8F\xB3To connect...",
    idrr = "\xE2\x9D\x97[Queue] Error: Failed to retrieve IDs, try restarting.",
    err = "\xE2\x9D\x97[Queue] There was an error",
    pos = "\xF0\x9F\x90\x8CYou are standing %d/%d in the queue\xF0\x9F\x95\x9C%s",
...
QBCore.Commands.Add("reloadqueuepriority", "Prioritize queue", {{name="id", help="Player ID"}, {name="priority", help="Priority level"}}, true, function(source, args)
	loadDatabaseQueue()
	TriggerClientEvent('chatMessage', source, "SYSTEM", "normal", "REFRESH")	
end, "god")

QBCore.Commands.Add("addpriority", "Add To Prioritize Queue", {{name="id", help="Player ID"}, {name="priority", help="Priority level"}}, true, function(source, args)
    local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
	local level = tonumber(args[2])
	if Player ~= nil then
        AddPriority(Player.PlayerData.source, level)
        TriggerClientEvent('chatMessage', source, "SYSTEM", "normal", "You have " .. GetPlayerName(Player.PlayerData.source) .. " given priority ("..level..")")	
	else
