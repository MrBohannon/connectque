...
function Queue:AddToConnecting(ids, ignorePos, autoRemove, done)
    local function remove()
        ...
        done(Config.Language.connectingerr .."\n🔸 Check our discord for more information: https://discord.gg/JtSTEhr")
...
function Queue:NotFull(firstJoin)
    local canJoin = Queue:ConnectingSize() +  Queue:GetPlayerCount() < Queue.MaxPlayers
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if Queue.DisplayQueueCount then
            SetConvarServerInfo("sv_queueCount", tostring(qCount))
            SetConvarServerInfo("sv_queueConnectingCount", tostring(Queue:ConnectingSize()))
            SetConvarServerInfo("sv_queueConnectedCount", tostring(Queue:GetPlayerCount()))
        end
    end
end)
